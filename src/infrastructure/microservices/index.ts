...
import { addImage } from './weaviate.ts'
...
app.post('/add-generation', async (req, res) => {
    try {
        ...
        const [prompt, image] = await Promise.all([inspectImg(url), fetchImg(url)].map(p => p.catch(e => e)))
        // 2. Add Goal Image to Weaviate(with generated prompt)
        addImage(image, prompt).then((res) => console.debug(`Added image to Weaviate`, res))
